# HABILITANDO ÁREA DE TRABALHO REMOTA
Vez ou outra precisaremos acessar nossa área de trabalho, as mais experientes recomendarão usar o 'ssh -x' ou usar 'xserver' e logar-se no ip de nosso desktop, no entanto, isso não é tão simples para novos usuários do linux e também não permite o acesso onde a origem é um desktop Windows. Portanto, no Debian, minha recomendação é instalar o xrdp, um protocolo de compartilhamento de sessões compativel com o 'rdp' da Microsoft e assim poderemos acessar nosso terminal Linux/Debian até mesmo de um Windows através do programa 'Remote Deskop'. 
...
### HABILITANDO ÁREA DE TRABALHO REMOTA - CRIANDO UM CERTIFICADO PARA O XRDP
